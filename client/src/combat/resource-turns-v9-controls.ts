...
<div class="combat-message" aria-live="polite"></div>`;
    ...
    public setCameraFocusControls(options: { enabled: boolean; player: { direction: 'left' | 'right' | null; stitching: number }; loomkeeper: { direction: 'left' | 'right' | null; stitching: number } }): void {
        ...
        for (const [selector, label, state] of [['.camera-focus-player', 'Back to You', options.player], ['.camera-focus-loomkeeper', 'Loomkeeper', options.loomkeeper]] as const) {
            ...
            if (state.direction) {
                ...
                button.setAttribute('aria-label', `${label}, ${state.stitching} Stitching, off-screen ${state.direction}`);
                if (usesV10R6ActionDynamics(this.state.rulesetId)) this.setCompactFocusCard(button, label, state.direction, state.stitching);
                else { button.removeAttribute('data-compact'); const mark = state.direction === 'left' ? '‹' : '›'; button.textContent = state.direction === 'left' ? `${mark} ${label} · ${state.stitching} Stitching` : `${label} · ${state.stitching} Stitching ${mark}`; }
            }
        }
    }
    ...
    private setCompactFocusCard(button: HTMLButtonElement, label: string, direction: 'left' | 'right', stitching: number): void {
        button.dataset.compact = 'true';
        let card = button.querySelector<HTMLElement>('.camera-focus-card');
        if (!card) {
            card = document.createElement('span'); card.className = 'camera-focus-card';
            const name = document.createElement('span'); name.className = 'camera-focus-name';
            const track = document.createElement('span'); track.className = 'camera-focus-track';
            const fill = document.createElement('span'); fill.className = 'camera-focus-fill'; track.appendChild(fill);
            const arrow = document.createElement('span'); arrow.className = 'camera-focus-arrow';
            card.append(name, track, arrow); button.replaceChildren(card);
        }
        card.querySelector<HTMLElement>('.camera-focus-name')!.textContent = label === 'Back to You' ? 'You' : 'Loom';
        const fill = card.querySelector<HTMLElement>('.camera-focus-fill')!;
        fill.style.width = `${Math.max(0, Math.min(100, stitching))}%`;
        fill.style.backgroundColor = stitchingHealthColor(stitching);
