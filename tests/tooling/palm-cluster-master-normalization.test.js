...
test('palm cluster source master integrity is portable and bound regeneration fails closed', () => {
  ...
  if (!fs.existsSync(config.source.default_external_path)) {
    assert.throws(() => norm(config.source.default_external_path, config), /ENOENT/);
    return;
  }
